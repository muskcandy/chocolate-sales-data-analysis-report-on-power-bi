# chocolate-sales-data-analysis-report-on-power-bi
🖼 Dashboard Preview
<img width="2361" height="1273" alt="Screenshot 2026-03-02 143254" src="https://github.com/user-attachments/assets/20ea0358-4dcd-4497-87ea-24ebf0577956" />

<br>
📌 Project Overview

This project presents an end-to-end data analysis and interactive dashboard built using Power BI to analyze chocolate sales performance across products, regions, time periods, and salespersons. The dashboard enables business stakeholders to quickly understand trends, identify top-performing products and regions, and monitor profitability.

The dataset spans from January 2023 to March 2025 and provides insights into:

Sales growth

Profitability

Shipment patterns

Regional performance

Product and salesperson contributions

<br>
<br>

🎯 Business Objectives

Track overall revenue and profit performance

Compare current year vs previous year trends

Identify top-selling and high-profit products

Analyze shipment distribution

Evaluate regional sales contribution

Measure salesperson performance

<br>
<br>

🛠 Tools & Technologies

Power BI – Data modeling, DAX, dashboard design

Microsoft Excel – Data source & preprocessing

DAX (Data Analysis Expressions) – KPI calculations

Data Visualization Techniques

<br>
<br>

📊 Key KPIs (From Dashboard)
Metric	Value
💰 Total Sales Amount	$141M
🚚 Total Shipments	25K
📦 Total Boxes	9M
📈 Profit Percentage	57.3%
💵 Total Profit	$81.10M
📈 Dashboard Insights
1️⃣ Sales Trend Analysis (Present Year vs Last Year)

Month-on-month comparison of current year vs last year

Helps identify seasonality and growth patterns

2️⃣ Shipment Distribution

Histogram visualization showing shipment counts vs box quantity

Reveals logistics and order-size distribution patterns

3️⃣ Product Performance

Top 6 Products by total revenue

Product-wise profit percentage

Identification of loss-making SKUs

4️⃣ Regional Sales Analysis

Country-wise sales distribution:

🇮🇳 India – 28.56%

🇳🇿 New Zealand – 21.27%

🇦🇺 Australia – 21.09%

🇺🇸 USA – 14.79%

🇬🇧 UK – 7.23%

🇨🇦 Canada – 7.07%

5️⃣ Salesperson Performance

Ranking of Top 6 Salespersons

KPIs:

Total sales

Total boxes shipped

Profit %

<br>
<br>

🧠 Key Learnings

Built dynamic KPIs using DAX

Designed business-focused interactive dashboards

Performed data cleaning and modeling

Applied data storytelling principles

Enhanced visual analytics and reporting skills

<br>
<br>

🚀 Future Enhancements

Add forecasting models

Integrate customer segmentation analysis

Implement drill-through analysis

Deploy dashboard to Power BI Service

<br>
<br>

👤 Author

Venu Sai Naik
📊 Data Analyst | SQL | Excel | Power BI | Python
